...
def process_documents():
    ...
    tokens_table = get_dictionary(unprocessed_doc_id)
...
def get_dictionary(unprocessed_doc_id):
    ...
    with open('project_files/elim.txt') as eliminator_file:
        ...
        for doc_id in unprocessed_doc_id:
            ...
            tokens_table = tokenize_document(data, int(doc_id), tokens_table)
    return tokens_table


def tokenize_document(data: str, doc_id: int, tokens_table: defaultdict):
    """
    :param data: content of a document.
    :param doc_id: ID of a document
    :param tokens_table: the whole dictionary of tokens.
    ...
    for token in tokens:
        tokens_table[token].append(doc_id)
    return tokens_table
